...
同步/异步 monitor 语义边界（第十七轮）：
- **同步（默认，async 省略或 false）**：在 set 调用线程上锁外内联执行，通知及时；回调抛异常会被框架吞掉记日志，不影响 set 返回结果，也不影响后续 monitor 执行。凡基于“当前状态”做判断/门控的监控必须用同步。
- **异步（async=true）**：任务投 Context 模块自持的监控池（固定单线程，与异步请求池独立，随 Context 服务 deinit 排空停止），属“延迟事件通知”：回调收到的是 set 时刻的数据快照，同一 key 连续 set 时异步回调处理的是当时的旧值而非最新值；事件顺序保证无条件成立（单线程保序，第十八轮），时效不保证。仅适合耗时回调（跨服务请求/落盘/通知）解耦，避免阻塞 set 调用方。
