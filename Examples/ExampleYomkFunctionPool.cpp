...
int main(int argc, char *argv[])
{
     ...
     */
    printStep(2, "带消息类型名的注册",
              "用 3 参形式注册 tool.greet 并传名调用；INFO_NAME 单查可见 [String] 元数据后缀。");
     ...
     */
    printStep(3, "无参调用与空参守卫",
              "传 nullptr 调用 tool.greet：函数内解包宏自动返回 eNo，调用方拿到统一契约消息。");
     ...
     */
    printStep(4, "同名热替换",
              "对 tool.greet 再注册一个 lambda 版本；再次调用返回新实现的结果。");
     ...
     */
    printStep(5, "内省三件套",
              "清单看全部、单查看一个、ALL 看数量与全量元信息；单查未注册名字演示 eNo。");
