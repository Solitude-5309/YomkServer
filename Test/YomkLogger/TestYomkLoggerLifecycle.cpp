...
// 级别开关：OFF→无输出仍 eOk→ON→恢复输出（四级别逐一）
{
    ...
    CoutCapture cap;
    CHECK(YOMK_INFO("lg1_s1_off_info_marker").m_status == YomkResponse::eOk, "INFO 级别关闭后调用仍 eOk");
    CHECK(cap.str().find("lg1_s1_off_info_marker") == std::string::npos, "INFO 级别关闭后无输出");
    CHECK(YOMK_ON_CONSOLE_LOG_INFO().m_status == YomkResponse::eOk, "ON_CONSOLE_LOG_INFO 返回 eOk");
    CoutCapture cap2;
    CHECK(YOMK_INFO("lg1_s1_on_info_marker").m_status == YomkResponse::eOk, "INFO 级别重开后调用 eOk");
    CHECK(cap2.str().find("lg1_s1_on_info_marker") != std::string::npos, "INFO 级别重开后恢复输出");
}
{
...
}
{
    ...
    CoutCapture cap;
    CHECK(
        YomkAPI::CONSOLE_LOG_INFO_TAG(longTag, "lg1_s2_longtag_marker").m_status == YomkResponse::eOk,
        "65536 字符超长 tag 返回 eOk");
    CHECK(cap.str().find("lg1_s2_longtag_marker") != std::string::npos, "超长 tag 日志正常输出");
    CHECK(cap.str().find("[" + longTag + "]") != std::string::npos, "超长 tag 完整回显到输出行");
}
{
    CoutCapture cap;
...
// ============ Section 2.5: 非法级别 default 降级（P2-c 修复后回填，须在 S3 设置 proxy 前） ============
{
    ...
    YomkResponse r(YomkResponse::eInvalid);
    {
        CoutCapture cap;
        r = YOMK_REQUEST(
            "/YomkLogger/console_log",
            YomkMkPtr(Log, yomk::Log{static_cast<yomk::Log::ELogLevel>(99), "lg1_s2p5_badlevel_marker", "MainLogger"}));
        out = cap.str();
    }
    CHECK(r.m_status == YomkResponse::eOk, "非法级别 99 console_log 返回 eOk（P2-c 修复后 default 活分支）");
    CHECK(
        out.find("unknown log level, use Info") != std::string::npos, "console_log default 分支错误提示（降级 Info）");
    CHECK(
        out.find("[Info ] [MainLogger] lg1_s2p5_badlevel_marker") != std::string::npos,
...
// ============ Section 3: proxy 契约（含 LG6/P4-e 卸载；末尾重装复位，S4-S7 仍为穿透态） ============
{
    ...
    CHECK(YOMK_SET_CONSOLE_LOG_PROXY(proxy).m_status == YomkResponse::eOk, "SET_CONSOLE_LOG_PROXY 返回 eOk");
    ...
    // 拦截态：proxy 返回 false → eOk + 专属消息 + 无控制台输出
    {
        ...
        CHECK(cap.str().find("lg1_s3_intercept_marker") == std::string::npos, "拦截态无默认控制台输出");
    ...
    // 穿透态：proxy 返回 true → 走默认输出
    {
        ...
        CHECK((r.m_status == YomkResponse::eOk && r.m_msg == "success."), "穿透态 console_log 返回 eOk + success.");
        CHECK(cap.str().find("lg1_s3_pass_marker") != std::string::npos, "穿透态恢复默认控制台输出");
    ...
    // 设置后 all 内省 proxy:on
    {
        ...
        YomkUnPackPkg(allResp.m_data, StringArray, arr);
        CHECK(
            (arr != nullptr && !arr->d.empty() && arr->d.front().find("proxy:on") != std::string::npos),
    ...
    // 与「本来就是穿透态」无从区分
    {
        ...
        CHECK(
            (r.m_status == YomkResponse::eOk && r.m_msg == "console log proxy is success."),
            "卸载前拦截态契约消息（代理确实在链路上）");
        CHECK(out.find("lg6_s3_pre_unload_marker") == std::string::npos, "卸载前拦截态无默认控制台输出");
    }

    // 卸载：传 nullptr（等价于空 std::function）
    {
        CHECK(
            YOMK_SET_CONSOLE_LOG_PROXY(nullptr).m_status == YomkResponse::eOk,
            "SET_CONSOLE_LOG_PROXY(nullptr) 卸载返回 eOk");
        auto allResp = YOMK_LOGGER_INFO_ALL();
        YomkUnPackPkg(allResp.m_data, StringArray, arr);
        CHECK(
            (arr != nullptr && !arr->d.empty() && arr->d.front().find("proxy:off") != std::string::npos),
    ...
    // 卸载后：恢复框架默认输出，且代理不再被调用
    {
        ...
        CHECK(
            (r.m_status == YomkResponse::eOk && r.m_msg == "success."),
    ...
    {
        g_proxyIntercept.store(false);
        CHECK(YOMK_SET_CONSOLE_LOG_PROXY(proxy).m_status == YomkResponse::eOk, "重装代理返回 eOk（卸载可逆）");
        auto allResp = YOMK_LOGGER_INFO_ALL();
        YomkUnPackPkg(allResp.m_data, StringArray, arr);
        CHECK(
            (arr != nullptr && !arr->d.empty() && arr->d.front().find("proxy:on") != std::string::npos),
...
// ============ Section 4: 文件日志闭环 ============
{
    ...
    CHECK(
        YOMK_FILE_LOG_CREATE(logDir.string(), "lg1_file_logger").m_status == YomkResponse::eOk,
        "FILE_LOG_CREATE 返回 eOk");
    CHECK((fs::exists(logFile) && fs::file_size(logFile, ec) == 0), "CREATE 后 <dir>/<name>.log 空文件已建");

    // 四级别入缓冲：write 前文件仍空（缓冲语义）
    CHECK(
        YOMK_FILE_INFO("lg1_file_logger", "lg1_s4_info_marker ", 7).m_status == YomkResponse::eOk,
        "FILE_INFO 返回 eOk");
    CHECK(YOMK_FILE_WARN("lg1_file_logger", "lg1_s4_warn_marker").m_status == YomkResponse::eOk, "FILE_WARN 返回 eOk");
    CHECK(
        YOMK_FILE_ERROR("lg1_file_logger", "lg1_s4_error_marker").m_status == YomkResponse::eOk, "FILE_ERROR 返回 eOk");
    CHECK(
        YOMK_FILE_DEBUG("lg1_file_logger", "lg1_s4_debug_marker").m_status == YomkResponse::eOk, "FILE_DEBUG 返回 eOk");
    CHECK(fs::file_size(logFile, ec) == 0, "WRITE 前文件仍为空（stringstream 缓冲语义）");

    // 落盘：读回断言行数/行格式/marker/tag
    CHECK(YOMK_FILE_LOG_WRITE("lg1_file_logger").m_status == YomkResponse::eOk, "FILE_LOG_WRITE 返回 eOk");
    bool readOk = false;
    ...
    CHECK(YOMK_FILE_LOG_WRITE("lg1_file_logger").m_status == YomkResponse::eOk, "自定义 tag 内容 WRITE eOk");
    ...
    CHECK(YOMK_FILE_LOG_WRITE("lg1_file_logger").m_status == YomkResponse::eOk, "二次 FILE_LOG_WRITE 返回 eOk");
    ...
    CHECK(
        (emptyBoth.m_status == YomkResponse::eInvalid && emptyBoth.m_msg == "logger name is empty."),
        "空名+空 dir 创建返回 eInvalid + logger name is empty.（P2-a 已处置）");
    auto emptyName = YOMK_FILE_LOG_CREATE(tmpDir.string(), "");
    CHECK(
        (emptyName.m_status == YomkResponse::eInvalid && emptyName.m_msg == "logger name is empty."),
        "空名+有效 dir 创建返回 eInvalid（P2-a 已处置）");
    auto emptyDir = YOMK_FILE_LOG_CREATE("", "lg1_empty_dir_name");
    CHECK(
        (emptyDir.m_status == YomkResponse::eInvalid && emptyDir.m_msg == "logger dir is empty."),
    ...
    CHECK(
        (badDir.m_status == YomkResponse::eNo && badDir.m_msg == "init file logger failed."),
        "不可创建 dir 返回 eNo + init file logger failed.（P2-b 已处置，异常不穿透）");
    auto ghostInfo = YOMK_LOGGER_INFO_LOGGER("lg1_bad_dir_logger");
    CHECK(
        (ghostInfo.m_status == YomkResponse::eNo && ghostInfo.m_msg == "logger not found."),
    ...
    CHECK(
        (retry.m_status == YomkResponse::eOk && fs::exists(retryDir / "lg1_bad_dir_logger.log")),
...
// ============ Section 5: 内省三端点 ============
{
    ...
    CHECK(
        (consoleInfo.m_status == YomkResponse::eOk && consoleInfo.m_msg == "MainLogger [console]"),
        "LOGGER(MainLogger) 命中 console 格式");
    auto fileInfo = YOMK_LOGGER_INFO_LOGGER("lg1_file_logger");
    CHECK(
        (fileInfo.m_status == YomkResponse::eOk &&
    ...
    CHECK(
        (noInfo.m_status == YomkResponse::eNo && noInfo.m_msg == "logger not found."),
    ...
    }
    {
        ...
        CHECK(
            (allArr2 != nullptr && !allArr2->d.empty() &&
...
{// 变参 fold：整数边界与浮点特殊值（ostringstream 默认格式字符串化）
 {CoutCapture cap;
CHECK(
    YOMK_INFO_TAG("lg1_s6_num", "i=", INT_MAX, " j=", INT_MIN, " z=", 0, " n=", -1).m_status == YomkResponse::eOk,
...
}
{
    ...
    CHECK(
        YOMK_INFO_TAG("lg1_s6_flt", "a=", nanV, " b=", infV, " c=", -infV, " d=", -0.0).m_status == YomkResponse::eOk,
...
// 1MB 超大内容：console + file 双路径
{
    ...
    CoutCapture cap;
    CHECK(YOMK_INFO_TAG("lg1_s6_big", big).m_status == YomkResponse::eOk, "1MB 超大内容控制台日志返回 eOk");
    CHECK(cap.str().find(big) != std::string::npos, "1MB 内容完整输出到控制台");

    fs::path bigDir = tmpDir / "s6";
    fs::create_directories(bigDir, ec);
    CHECK(
        YOMK_FILE_LOG_CREATE(bigDir.string(), "lg1_big_logger").m_status == YomkResponse::eOk,
        "超大内容用 file logger 创建 eOk");
    CHECK(YOMK_FILE_INFO("lg1_big_logger", big).m_status == YomkResponse::eOk, "1MB 超大内容文件日志返回 eOk");
    CHECK(YOMK_FILE_LOG_WRITE("lg1_big_logger").m_status == YomkResponse::eOk, "超大内容 WRITE 返回 eOk");
    bool readOk = false;
    std::string content = readFile(bigDir / "lg1_big_logger.log", readOk);
    CHECK((readOk && content.find(big) != std::string::npos), "1MB 内容完整落盘");
}
...
// （console_log 的 default 已在 S2.5 于 proxy 设置前触达）
{
    ...
    CHECK(
        YOMK_FILE_LOG_CREATE(badLevelDir.string(), "lg1_s6_badlevel").m_status == YomkResponse::eOk,
    ...
    YomkResponse r(YomkResponse::eInvalid);
    {
        ...
        r = YOMK_REQUEST(
            "/YomkLogger/file_log",
            YomkMkPtr(
                Log, yomk::Log{static_cast<yomk::Log::ELogLevel>(99), "lg1_s6_badlevel_marker", "lg1_s6_badlevel"}));
        out = cap.str();
    }
    CHECK(
        (r.m_status == YomkResponse::eOk && r.m_msg == "success."),
        "file_log 非法级别 99 返回 eOk（P2-c 修复后 default 活分支）");
    CHECK(out.find("unknown log level, use Info") != std::string::npos, "file_log default 分支错误提示（降级 Info）");
    CHECK(YOMK_FILE_LOG_WRITE("lg1_s6_badlevel").m_status == YomkResponse::eOk, "非法级别落盘 WRITE eOk");
    bool readOk = false;
    std::string content = readFile(badLevelDir / "lg1_s6_badlevel.log", readOk);
    CHECK(
        (readOk && countLines(content) == 1 && content.find("[Info ] lg1_s6_badlevel_marker") != std::string::npos),
        "非法级别降级 Info 落盘恰 1 行");
    ...
    YomkResponse rOff(YomkResponse::eInvalid);
    {
        CoutCapture cap;
        rOff = YOMK_REQUEST(
            "/YomkLogger/off_console_log_by_level",
            YomkMkPtr(Log, yomk::Log{static_cast<yomk::Log::ELogLevel>(99), "", ""}));
        outOff = cap.str();
    }
    CHECK(
        (rOff.m_status == YomkResponse::eOk && outOff.find("unknown log level, turn off failed.") != std::string::npos),
    ...
    YomkResponse rOn(YomkResponse::eInvalid);
    {
        CoutCapture cap;
        rOn = YOMK_REQUEST(
            "/YomkLogger/on_console_log_by_level",
            YomkMkPtr(Log, yomk::Log{static_cast<yomk::Log::ELogLevel>(99), "", ""}));
        outOn = cap.str();
    }
    CHECK(
        (rOn.m_status == YomkResponse::eOk && outOn.find("unknown log level, turn on failed.") != std::string::npos),
...
// ============ Section 7: 异常路径（空包 / 错类型包 / 未知端点） ============
{
    ...
    CHECK(YOMK_REQUEST("/YomkLogger/file_log", nullptr).m_status == YomkResponse::eNo, "file_log nullptr 包返回 eNo");
    CHECK(
        YOMK_REQUEST("/YomkLogger/write_file_log", nullptr).m_status == YomkResponse::eNo,
        "write_file_log nullptr 包返回 eNo");
    CHECK(
        YOMK_REQUEST("/YomkLogger/create_file_logger", nullptr).m_status == YomkResponse::eNo,
        "create_file_logger nullptr 包返回 eNo");
    CHECK(YOMK_REQUEST("/YomkLogger/logger", nullptr).m_status == YomkResponse::eNo, "logger nullptr 包返回 eNo");
    ...
    // 注：CHECK 须在捕获作用域外（否则断言输出被 rdbuf 重定向吞没）
    {
        ...
        YomkResponse r(YomkResponse::eInvalid);
        {
            ...
            r = YOMK_REQUEST(
                "/YomkLogger/console_log",
                YomkMkPtr(Log, yomk::Log{yomk::Log::eInfo, "lg1_s7_empty_logger_marker", ""}));
            out = cap.str();
        }
        CHECK(r.m_status == YomkResponse::eOk, "console_log 空 logger 名返回 eOk（回退分支）");
        CHECK(
            out.find("console logger name is empty, use MainLogger") != std::string::npos,
            "空 logger 名错误提示（回退 MainLogger）");
        CHECK(
            out.find("[MainLogger] lg1_s7_empty_logger_marker") != std::string::npos,
    ...
    // file_log 空 logger 名 → eNo 分支（宏形态 file 参可传空串，raw 注入等价触达）
    {
        ...
        {
            CoutCapture cap;
            r = YOMK_REQUEST("/YomkLogger/file_log", YomkMkPtr(Log, yomk::Log{yomk::Log::eInfo, "x", ""}));
            out = cap.str();
        }
        CHECK(
            (r.m_status == YomkResponse::eNo && r.m_msg == "file logger name is empty."),
            "file_log 空 logger 名返回 eNo + 契约消息");
        CHECK(out.find("file logger name is empty.") != std::string::npos, "file_log 空名错误提示");
    }
    // LOGGER_INFO_LOGGER 空名 → eNo（LG2 堵死空名注册后回填；
    // LG1 时为 P2-a 幽灵 logger 命中 eOk 的现状固化）
    auto emptyNameInfo = YOMK_LOGGER_INFO_LOGGER("");
    CHECK(
        (emptyNameInfo.m_status == YomkResponse::eNo && emptyNameInfo.m_msg == "logger not found."),
...
// ============ Section 8: logger 删除接口（LG4 新增 /delete_logger 全语义） ============
{
    ...
    // 8.1 空名 → eInvalid（与 createFileLogger 的 P2-a 空名惯例同口径）
    {
        ...
        CHECK(
            (r.m_status == YomkResponse::eInvalid && r.m_msg == "logger name is empty."),
            "S8: 删除空名返回 eInvalid + 契约消息");
        CHECK(out.find("logger name is empty.") != std::string::npos, "S8: 空名删除错误提示");
    }
    ...
    // 8.2 两表均未命中 → eNo + not-found 契约消息（对齐 writeFileLog/loggerInfo）
    {
        ...
        CHECK(
            (r.m_status == YomkResponse::eNo && r.m_msg == "logger not found."),
            "S8: 删除未注册 logger 返回 eNo + logger not found.");
        CHECK(
            out.find("logger: lg1_s8_never_created not found.") != std::string::npos, "S8: 未命中错误提示含 logger 名");
    }
    ...
    // 8.3 console-only：msg 计数 1/0；删除后单查 eNo；同名再写惰性重建
    {
        ...
        CHECK(
            consoleInfo(name, "lg1_s8_console_marker").m_status == YomkResponse::eOk, "S8: console-only 预热创建 eOk");
        CHECK(YOMK_LOGGER_INFO_LOGGER(name).m_status == YomkResponse::eOk, "S8: 创建后单查命中 eOk");

        auto del = YOMK_FILE_LOG_DELETE(name);
        CHECK(
            (del.m_status == YomkResponse::eOk && del.m_msg == "deleted console:1 file:0"),
            "S8: console-only 删除 eOk + msg 计数: " + del.m_msg);
        CHECK(YOMK_LOGGER_INFO_LOGGER(name).m_status == YomkResponse::eNo, "S8: console 删除后单查 eNo");
        // 惰性重建：同名再写一条 → consoleLog 双检锁重建路径
        CHECK(
            consoleInfo(name, "lg1_s8_rebuild_marker").m_status == YomkResponse::eOk,
            "S8: 删除后同名再写 eOk（惰性重建）");
        CHECK(YOMK_LOGGER_INFO_LOGGER(name).m_status == YomkResponse::eOk, "S8: 惰性重建后单查恢复 eOk");
        CHECK(YOMK_FILE_LOG_DELETE(name).m_status == YomkResponse::eOk, "S8: 清理重建 logger eOk");
    }
    ...
    // 8.4 file-only：写 2 行不 flush → 删除释放最后引用 → ~FileLogger 锁外自动落盘
    {
        ...
        CHECK(YOMK_FILE_LOG_CREATE(dir.string(), name).m_status == YomkResponse::eOk, "S8: file logger 创建 eOk");
        CHECK(
            YOMK_FILE_INFO_TAG(name, "s8Tag", "lg1_s8_file_marker_1").m_status == YomkResponse::eOk,
            "S8: file 写入第 1 行 eOk");
        CHECK(
            YOMK_FILE_INFO_TAG(name, "s8Tag", "lg1_s8_file_marker_2").m_status == YomkResponse::eOk,
            "S8: file 写入第 2 行 eOk");
        // 前置对照：未 flush 前 init 建的空文件仍为空
        bool okBefore = false;
        std::string contentBefore = readFile(dir / (name + ".log"), okBefore);
        CHECK((okBefore && contentBefore.empty()), "S8: 删除前未 flush，文件仍为空");

        auto del = YOMK_FILE_LOG_DELETE(name);
        CHECK(
            (del.m_status == YomkResponse::eOk && del.m_msg == "deleted console:0 file:1"),
            "S8: file-only 删除 eOk + msg 计数: " + del.m_msg);
        bool okAfter = false;
        std::string content = readFile(dir / (name + ".log"), okAfter);
        CHECK((okAfter && countLines(content) == 2), "S8: 删除触发析构自动落盘恰 2 行");
        CHECK(
            (content.find("lg1_s8_file_marker_1") != std::string::npos &&
             content.find("lg1_s8_file_marker_2") != std::string::npos),
            "S8: 落盘内容含两条 marker（数据不因删除丢失）");
        CHECK(YOMK_FILE_LOG_WRITE(name).m_status == YomkResponse::eNo, "S8: 删除后 WRITE 同名返回 eNo");
    }
    ...
    // 8.5 同名双表：一次删除同时清理两表（msg 计数 1/1），重复删除幂等 eNo
    {
        ...
        CHECK(
            consoleInfo(name, "lg1_s8_both_console_marker").m_status == YomkResponse::eOk,
            "S8: 双表用例 console 侧创建 eOk");
        CHECK(YOMK_FILE_LOG_CREATE(dir.string(), name).m_status == YomkResponse::eOk, "S8: 双表用例 file 侧创建 eOk");

        auto del = YOMK_FILE_LOG_DELETE(name);
        CHECK(
            (del.m_status == YomkResponse::eOk && del.m_msg == "deleted console:1 file:1"),
            "S8: 同名双表一次删除 eOk + msg 计数: " + del.m_msg);
        CHECK(YOMK_LOGGER_INFO_LOGGER(name).m_status == YomkResponse::eNo, "S8: 双表删除后单查 eNo（两表均已清理）");
        std::string out2;
        ...
        CHECK(
            (del2.m_status == YomkResponse::eNo && del2.m_msg == "logger not found."), "S8: 重复删除返回 eNo（幂等）");
        CHECK(
            out2.find("logger: lg1_s8_both_tables not found.") != std::string::npos,
    ...
    // 8.6 内省行数递减守恒 + ALL 与 LOGGERS 行数关系 + MainLogger 删除后惰性重建
    {
        ...
        CHECK(
            consoleInfo(name, "lg1_s8_count_marker").m_status == YomkResponse::eOk, "S8: 计数用例 console 侧创建 eOk");
        CHECK(YOMK_FILE_LOG_CREATE(dir.string(), name).m_status == YomkResponse::eOk, "S8: 计数用例 file 侧创建 eOk");
        CHECK(unpackLines(YOMK_LOGGER_INFO_LOGGERS()).size() == before + 2, "S8: 双表各建一个 → LOGGERS 行数 +2");
        CHECK(YOMK_FILE_LOG_DELETE(name).m_status == YomkResponse::eOk, "S8: 计数用例删除 eOk");
        CHECK(unpackLines(YOMK_LOGGER_INFO_LOGGERS()).size() == before, "S8: 删除后 LOGGERS 行数回落（递减守恒）");
        auto allLines = unpackLines(YOMK_LOGGER_INFO_ALL());
        CHECK(
            (allLines.size() == before + 1 && allLines.front() == "console:debug:on info:on warn:on error:on proxy:on"),
        ...
        CHECK(YOMK_LOGGER_INFO_LOGGER("MainLogger").m_status == YomkResponse::eNo, "S8: MainLogger 删除后单查 eNo");
        std::string rebuildOut;
        YomkResponse rebuild(YomkResponse::eInvalid);
        {
            CoutCapture cap;
            rebuild = YOMK_REQUEST(
                "/YomkLogger/console_log",
                YomkMkPtr(Log, yomk::Log{yomk::Log::eInfo, "lg1_s8_mainlogger_rebuild_marker", ""}));
            rebuildOut = cap.str();
        }
        CHECK(rebuild.m_status == YomkResponse::eOk, "S8: MainLogger 删除后空名回退写入 eOk");
        CHECK(
            YOMK_LOGGER_INFO_LOGGER("MainLogger").m_status == YomkResponse::eOk,
            "S8: MainLogger 惰性重建后单查恢复 eOk");
        CHECK(
            rebuildOut.find("[MainLogger] lg1_s8_mainlogger_rebuild_marker") != std::string::npos,
            "S8: 重建对象名确为 MainLogger（输出行 [MainLogger]）");
        CHECK(
            unpackLines(YOMK_LOGGER_INFO_LOGGERS()).size() == before,
    ...
    // 8.7 异常路径：空包 / 错类型包（YomkUnPackPkgResponse 契约分支）
    {
        ...
        CHECK(
            (rNull.m_status == YomkResponse::eNo && rNull.m_msg == " pkg is null or pkg is not String. "),
            "S8: delete_logger nullptr 包返回 eNo + 契约消息");
        auto rWrong = YOMK_REQUEST("/YomkLogger/delete_logger", YomkMkPtr(Log, yomk::Log{yomk::Log::eInfo, "x", "y"}));
        CHECK(
            (rWrong.m_status == YomkResponse::eNo && rWrong.m_msg == " pkg is null or pkg is not String. "),
