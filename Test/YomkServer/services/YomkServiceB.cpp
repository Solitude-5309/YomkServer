...
int YomkServiceB::init()
{
    ...
    // 注册到外部子系统 FunctionPool：弱绑定成员回调，服务删除后回调自动失效，不会悬垂 this 崩溃
    YOMK_FUNCTIONPOOL_REGISTER("service_b_work", YomkBindWeakSelf(YomkServiceB::callSkillB));

    // 日志：记录安装的功能函数名和服务名
    YOMK_INFO_TAG("YomkServiceB::init", "install func [ /call_skill_b ] and register [ service_b_work ] to", name());
