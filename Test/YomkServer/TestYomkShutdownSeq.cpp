...
{
public:
    DrainSrv(YomkServer* server) : YomkService(server) {}
...
private:
    YomkResponse ping(YomkPkgPtr pkg) { return {YomkResponse::eOk, "pong"}; }
    YomkResponse work(YomkPkgPtr pkg) { return {YomkResponse::eOk, "work"}; }
};
...
{
public:
    SpawnSrv(YomkServer* server) : YomkService(server), m_server(server) {}
...
static void testDrainSemantics()
{
    ...
    CHECK(
        g_deletedOnDeinit.load() == false,
        "排空语义：deinit 时 deleted() 为 false（shutdown 不置 markDeleted，与删除即停对比）");
    CHECK(probed.load() == true, "排空：弱绑定探针回调已执行");
    CHECK(
        g_weakRespDuringDrain.m_status == YomkResponse::eOk && g_weakRespDuringDrain.m_msg == "work",
...
static void testRejectAfterShutdown()
{
    ...
    server->asyncRequest(
        "/DrainSrv/ping", nullptr, [&rejectCbCount](YomkResponse resp) { rejectCbCount.fetch_add(1); });
    std::this_thread::sleep_for(std::chrono::milliseconds(200));
...
static void testDestructorFallback()
{
    resetGlobals();
    {
        ...
        for (int i = 0; i < 20; ++i)
        {
            server->asyncRequest("/DrainSrv/ping", nullptr, [](YomkResponse resp) { g_taskCount.fetch_add(1); });
        }
