...
int main()
{
            ...
            workers.emplace_back(
                [&, t]
                {
                    for (int i = 0; i < K; ++i)
                    {
                        ...
                        {
                            auto r = YOMK_CONTEXT_SET(
                                "conc_rw",
                                YomkMkPtr(String, std::string("v") + std::to_string(t) + "_" + std::to_string(i)));
        ...
        for (int t = 0; t < T; ++t)
        {
            workers.emplace_back(
                [&]
                {
                    for (int i = 0; i < K; ++i)
                    {
                        auto r = YOMK_CONTEXT_SET_MONITOR(
                            "conc_mon", [](const yomk::Context&) { ++g_syncMon; }, /*async=*/false);
                        if (r.m_status == YomkResponse::eOk)
                            ++okMon;
                        else
                            ++g_illegal;
            ...
            workers.emplace_back(
                [&, writer]
                {
                    for (int i = 0; i < K; ++i)
                    {
                        if (writer)
                        {
                            auto r = YOMK_CONTEXT_SET_CHECKER(
                                "conc_ck", [](const yomk::Context&) { return ContextChecker::eAccept; });
                            if (r.m_status == YomkResponse::eOk)
                                ++okWriter;
                            else
                                ++g_illegal;
        ...
        for (int t = 0; t < T; ++t)
        {
            workers.emplace_back(
                [&]
                {
                    ...
                    {
                        auto r =
                            YOMK_CONTEXT_SET("conc_setck", YomkMkPtr(String, std::string("s") + std::to_string(i)));
                        if (r.m_status == YomkResponse::eOk)
                            ++okSet;
                        else
                            ++g_illegal;
    ...
    std::cout << "===== 6. 并发 set 触发 sync+async monitor =====" << std::endl;
    {
        ...
        YOMK_CONTEXT_SET_MONITOR(
            "conc_setmon", [](const yomk::Context&) { ++g_syncMon; }, /*async=*/false);
        YOMK_CONTEXT_SET_MONITOR(
            "conc_setmon", [](const yomk::Context&) { ++g_asyncMon; }, /*async=*/true);
        YOMK_CONTEXT_ON_MONITOR();
        ...
        for (int t = 0; t < T; ++t)
        {
            workers.emplace_back(
                [&]
                {
                    ...
                    {
                        auto r =
                            YOMK_CONTEXT_SET("conc_setmon", YomkMkPtr(String, std::string("n") + std::to_string(i)));
                        if (r.m_status == YomkResponse::eOk)
                            ++okSet;
                        else
                            ++g_illegal;
        ...
        bool asyncOk = waitFor([&] { return g_asyncMon.load() == T * K; }, 10000, "async monitor count");
    ...
    std::cout << "===== 8. N7 压力（≈10 万级混合操作） =====" << std::endl;
    {
        ...
        for (int n = 0; n < M; ++n)
        {
            auto cr = YOMK_CONTEXT_CREATE(
                "stress_" + std::to_string(n), YomkMkPtr(String, std::string("init") + std::to_string(n)));
                ...
                [&, t]
                {
                    for (int i = 0; i < K; ++i)
                    {
                        ...
                        switch (op)
                        {
                            ...
                            {
                                auto r = YOMK_CONTEXT_SET_CHECKER(
                                    key, [](const yomk::Context&) { return ContextChecker::eAccept; });
                                if (r.m_status == YomkResponse::eOk)
                                    ++okOps;
                                else
                                    ++g_illegal;
